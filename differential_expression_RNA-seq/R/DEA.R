...
files <- file.path(dir,opt$dir_quants, samples$names, "abundance.h5")
names(files) <- paste0(samples$name_id)
...
write.csv(as.data.frame(resOrd2),file = opt$outdir_cvs)

# Exportar las tabla con los genes DE sin filtrar
write.csv(as.data.frame(resOrd2_a),file = opt$outres_cvs)

# Exportar las tabla con los genes DE filtrados
write.csv(as.data.frame(resOrd2_f),file = opt$outfilt_cvs)

# Exportar la matriz de cuentas resumida con tximpor de los archivos de abundace de kallisto
write.csv(as.data.frame(txi.kallisto$counts),file=opt$countsmat)

# Exportar la matriz de cuentas TPM resumida con tximpor de los archivos de abundace de kallisto
write.csv(as.data.frame(txi.countpm$counts),file=opt$countpm)
